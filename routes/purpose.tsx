...
import { Clock, AlertCircle, Zap, Upload, Target, Users, MessageSquare, BarChart3 } from "lucide-react";

export const Route = createFileRoute("/purpose")({
  head: () => ({
    meta: [
      { title: "Purpose — Prospect Compass" },
      { name: "description", content: "Why Prospect Compass exists: cut daily prospecting to under one hour." },
    ],
  }),
  component: () => (
    <AuthGuard>
      ...
      <section>
        ...
        <div className="mt-6 relative overflow-hidden rounded-xl border bg-gradient-to-br from-primary to-primary/80 text-primary-foreground p-6">
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div>
              <div className="text-[11px] uppercase tracking-wider opacity-80 font-semibold">Must have</div>
              <div className="text-lg font-semibold mt-0.5">Cut daily prospecting to under one hour.</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-5">
          ...
          <div className="rounded-xl border bg-card p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">The Solution</h2>
            </div>
            <ol className="space-y-3">
              {[
                { icon: Upload, title: "Upload & adjust your ICP", desc: "Define your ideal customer profile in seconds." },
                { icon: Target, title: "Automated reading & scoring", desc: "Surfaces the hottest companies matched to your ICP." },
                { icon: Users, title: "5 key contacts per company", desc: "LinkedIn profile links delivered automatically." },
                { icon: MessageSquare, title: "5-message sequences", desc: "Tailored to each news trigger and contact's role." },
                { icon: BarChart3, title: "Activity dashboard", desc: "Track by company or contact, with full metrics." },
              ].map((s, i) => {
                const Icon = s.icon;
                return (
